...
@Component({
  selector: 'app-rehabilitation-registration',
  templateUrl: './rehabilitation-registration.component.html',
  styleUrls: ['./rehabilitation-registration.component.css']
})
export class RehabilitationRegistrationComponent implements OnInit {
  page = {
    total: 0,
    size: 10,
    index: 1
...
// 添加按钮
 ...
 turnToAdd(data,e?: MouseEvent){
  ...
  if(data==""){
    this.router.navigate(['rehabilitationRegistrationAdd'], { relativeTo: this.route });
  }else{
    this.router.navigate(['rehabilitationRegistrationAdd',{info: JSON.stringify(data)}], { relativeTo: this.route });
  }
}
...
turnToCheck(data,e?: MouseEvent){
  ...
  this.router.navigate(['rehabilitationRegistrationCheck',{info: JSON.stringify(data)}], { relativeTo: this.route });
}
  // 删除
