...
@Component({
  selector: 'app-rehabilitation-person-middle',
  templateUrl: './rehabilitation-person-middle.component.html',
  styleUrls: ['./rehabilitation-person-middle.component.css']
})
export class RehabilitationPersonMiddleComponent implements OnInit {
	page = {
		total: 0,
		size: 10,
		index: 1
	...
	updateList(reset: boolean = false): void {
		...
		this.httpReq.post('/rcMidEvaluation/listPage', null, this.reqObj, (data) => {
	...
	turnToEdit(data, e?: MouseEvent){
		...
		this.router.navigate([ 'PersonMiddleAdd', { recureCaseId:data.recureCase.id,recureFlag:data.recureCase.recureFlag } ], {
	...
	turnToCheck(data, e?: MouseEvent){
		...
		this.router.navigate([ 'PersonMiddleCheck', { recureCaseId:data.recureCase.id,recureFlag:data.recureCase.recureFlag } ], {
			relativeTo: this.route
		});
	}
	...
	delete(id, e?: MouseEvent) {
		...
		this.globalService.delModal(() => {
			this.httpReq.post('/rcMidEvaluation/del', null, { id: id }, (data) => {
				if (data['status'] == 200) {
					if (data['code'] == 0) {
