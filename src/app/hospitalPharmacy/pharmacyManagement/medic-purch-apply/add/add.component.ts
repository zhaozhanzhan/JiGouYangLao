...
export class AddComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.getParam.state == "edit" || this.getParam.state == "detail") {
  ...
  public selDrug(ev: any) {
    ...
    if (!_.isNull(ev) && !_.isUndefined(ev) && ev.length > 2) {
      ...
      drugObj = {
        ...
        lastPurchasePrice: drugArr[2] //	最近一次采购单价
      };
    } else {
      drugObj = {
        medName: "", //	String	药品名称
        medId: "", //	String	药品id
        lastPurchasePrice: "" //	Double	单价
      };
    }
  ...
  public editDrug(obj: any, state: string = "edit", i: number) {
    ...
    for (let i = 0; i < this.goodsList.length; i++) {
      this.selIdArr.push(this.goodsList[i].medId);
    }
