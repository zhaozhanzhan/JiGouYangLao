...
@Component({
  selector: "app-vein-med-list",
  templateUrl: "./vein-med-list.component.html",
  styleUrls: ["./vein-med-list.component.css"]
})
export class VeinMedListComponent implements OnInit {
  sendData = {
    /**
    ...
    this.httpReq.post(
      ...
      data => {
        ...
        if (data.code == 0) {
          ...
          // this.listOfData.sort((a,b)=>{return a.admissionNo-b.admissionNo});
          ...
          this.listOfData.forEach((item, index, arr) => {
            ...
            // item['proJect'].push({drugName:item.drugName,unitStr:item.unitStr,drugFrequencyStr:item.drugFrequencyStr,westernMedicineOrdersId:item.westernMedicineOrdersId});
          });
        }
      }
    );
