...
@Component({
  selector: "app-onHand-temporaryOrder",
  templateUrl: "./onHandTemporaryOrder.component.html",
  styleUrls: ["./onHandTemporaryOrder.component.css"]
})
export class OnHandTemporaryOrderComponent implements OnInit {
  @Input("patientId")
...
.poBottom {
  ...
  saveWesternMedicine() {
    ...
    if (this.tabIndex == "2") {
      ...
      // const frequencyNameList = this.westernMedicine.frequencyName.split(",");
      // let fNameStr = "";
      // frequencyNameList.forEach(e => {
      //   fNameStr += `<span>${e}</span><br>`;
      // });
  ...
  confirmOrder() {
    ...
    // 下达医嘱
    this.httpReq.post("/temp_orders/save", null, obj, data => {
      this.isVisible = false;
  ...
  edit(state, data, i) {
    ...
    this.patientsBasicInfo.attendingDoc = data.inHospitalInfo.attendingDoc;
    this.patientsBasicInfo.admissionNo = data.inHospitalInfo.admissionNo;
    ...
    if (result.tabIndex == "2") {
      ...
      console.log(this.westernMedicine.drugList);
