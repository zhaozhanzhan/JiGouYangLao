...
@Component({
  ...
  styleUrls: ["./specialOrder.component.css"]
})
export class SpecialOrderComponent implements OnInit {
  ...
  /**
   * 获取详情
   * @memberof SpecialOrderComponent
   */
  public getInfo() {
    const reqObj: any = {};
    reqObj.inHospitalInfoId = this.patientId; // 病人基本信息id
    this.httpReq.post("special_hy_report/list", null, reqObj, (data: any) => {
      if (data["code"] == 0) {
  ...
  public saveForm() {
    ...
    this.httpReq.post("special_hy_report/save", null, sendData, (data: any) => {
      if (data["code"] == 0) {
