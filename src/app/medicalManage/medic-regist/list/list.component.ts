...
export class ListComponent implements OnInit {
  ...
  // hasSendOver: "0" // 当天是否服药完成 0未完成/1已完成
  ...
  public formData: any = {
    idArr: "", // 所勾选待服药老人id(多个老人id拼String用逗号隔开)
    accountId: "" // 账户id
  }; // 要提交给后台的数据对象
  // receiver: "", // 领用人
  ...
  public serFun(isRest: boolean = false) {
    ...
    this.httpReq.post("takeMedList", null, this.serValObj, data => {
  ...
  public handleOk(): void {
    ...
    this.httpReq.post("dtTakeMed", null, sendObj, (data: any) => {
      if (data["code"] == 0) {
        this.isOkLoading = false; // 正在提交
        this.isVisible = false;
        this.msg.success("批量服药成功！");
        this.serFun(); // 刷新列表
      } else {
        if (data["message"]) {
          // this.msg.error(data["message"]);
