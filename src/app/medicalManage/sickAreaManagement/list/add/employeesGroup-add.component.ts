...
export class EmployeesGroupAddComponent implements OnInit {
  ...
  ngOnInit() {
    this.validateForm = this.fb.group({
      ...
      sickWardNo: [null],
      sickWardTel: [null],
      // sectionOfficeId: [null, [Validators.required]]
    });
    ...
    if (employeesGroupStr) {
      ...
      this.validateForm.patchValue({ name: this.employeesGroup.sickWardName });
      this.validateForm.patchValue({ sickWardNo: this.employeesGroup.sickWardNo });
      this.validateForm.patchValue({ sickWardTel: this.employeesGroup.sickWardTel });
    ...
    //     if (
    //       this.employeesGroup &&
    //       Utils.isNotEmpty(this.employeesGroup.sectionOfficeId)
    //     ) {
    //       this.validateForm.patchValue({
    //         sectionOfficeId: this.employeesGroup.sectionOfficeId.split(",")
    //       });
    //     }
    //   }
    // });
  ...
  onSubmit() {
    ...
    // reqObj.sectionOfficeId = reqObj.sectionOfficeId.join(",");
    // reqObj.roomIds = this.roomIds;
    // reqObj.roomNames = this.roomNames;

    // delete reqObj.sectionOfficeId;
    // delete reqObj.value;
    ...
    if (!this.employeesGroup || !this.employeesGroup.id) {
      this.httpReq.post("sickWard/add", null, reqObj, data => {
      ...
      this.httpReq.post("sickWard/edit", null, reqObj, data => {
        this.isSaveBtnLoading = false;
        if (data["status"] == 200) {
          this.isSaveBtnLoading = false;
          this.message.success("保存成功！");
