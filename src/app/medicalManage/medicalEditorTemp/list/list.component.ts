...
export class ListComponent implements OnInit {
  ...
  addTemplate(): void {
    ...
    this.httpReq.post("/disease_template/save", null, params, data => {
  ...
  delTemplate(contract) {
    ...
    let that = this;
    this.httpReq.post("/disease_template/del", null, params, data => {
      that.isTableLoading = false;
      if (data["code"] == 0) {
        that.loadingDataArray(true);
      }
  ...
  toggerTemoState(contractInfo, isRelease: boolean, e?: MouseEvent) {
    ...
    if (isRelease) {
      contractInfo.startStop = 1;
    } else {
      contractInfo.startStop = 2;
    }
    ...
    let that = this;
    this.httpReq.post("/disease_template/edit", null, contractInfo, data => {
      that.isSaveLoading = false;
      if (data["code"] == 0) {
        that.isShowAddDialog = false;
