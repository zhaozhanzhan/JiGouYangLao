...
@Component({
  selector: "app-showForAfterIn",
  templateUrl: "./showForAfterIn.component.html",
  styleUrls: ["./showForAfterIn.component.css"]
})
export class ShowForAfterInComponent implements OnInit {
  //当前页面显示状态
  pageMode = "";
  departmentList = [];
    ...
    if (!Utils.isEmpty(requestId)) {
      ...
      this.httpReq.post("requisition/listDetail", null, reqParam, data => {
        if (data["code"] == 0) {
          ...
          this.createDate = resultData.createDate.split(" ")[0];
          resultData.requisitionGoods.forEach(element => {
            let tempGoodInfo = {
              ...
              isInWh: element.isInWh,
              inWhName: element.inWhName ? element.inWhName : ""
            };
