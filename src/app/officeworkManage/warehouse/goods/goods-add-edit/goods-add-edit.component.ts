...
export class GoodsAddEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (goodsStr) {
      ...
      this.goodsCategoryName = goods.goodsCategory.name;
      this.goodsId = goods.id;
      this.validateForm.patchValue(goods);
      this.validateForm.patchValue({ categoryId: goods.goodsCategory.id });
    }
  ...
  saveForm() {
    ...
    this.httpReq.post("goods/saveOrEdit", null, reqObj, data => {
