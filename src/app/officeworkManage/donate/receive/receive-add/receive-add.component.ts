...
@Component({
  selector: "app-receive-add",
  templateUrl: "./receive-add.component.html",
  styleUrls: ["./receive-add.component.css"]
})
export class ReceiveAddComponent implements OnInit {
  ...
  onRecDeptChange(recDeptId: string): void {
    ...
    this.validateForm.patchValue({ recWhId: "" });
    const reqObj = {
      deptId: recDeptId,
  ...
  saveForm(operatorUrl, receiverUrl) {
    ...
    this.httpReq.post("outDonatedWarehouse/saveRec", null, reqObj, data => {
