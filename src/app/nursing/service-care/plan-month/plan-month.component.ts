...
export class PlanMonthComponent implements OnInit {
  ...
  // 点击某时间段获取相关信息
  getInTimeInfo(day){
    // {"oldmanId": "老人id","weekNum": "第几周"}
    this.InTimeInfo = day;
    this.inTimeInfoLoading = true;
    this.httpReq.post('care_service/listMonth',null,{oldmanId:this.planid,weekNum:day},data=>{
  ...
  addServiceModelOk(){
    ...
    let sendData = {
      // {"oldmanId": "老人id","weekNum": "第几周","projectIds":"服务项目ids","accountId": "账户id"}
    ...
    this.httpReq.post('care_service/saveMonth',null,sendData,data=>{
  ...
  getServiceList(renewPage:boolean = false){
    ...
    this.checkServiceArr = [];
    this.isPartSel = this.allChecked = false;
    this.httpReq.post("care_service/listForMonth", null, this.sendData, data => {
      this.addServiceModelLoading = false;
      if (data.code == 0) {
  ...
  delete(id,proName){
    if(id){
      this.modalService.confirm({
        ...
        nzOnOk: () => {
          this.inTimeInfoLoading = true;
          // {"oldmanId":"老人Id","weekNum": "周几","projectIds":"服务项目ids","accountId": "账户id"}
          let sendData = {
            oldmanId: this.planid,
            weekNum: this.InTimeInfo,
            projectIds: id,
          ...
          this.httpReq.post("care_service/deleteMonth",null,sendData,data => {
            this.inTimeInfoLoading = false;
              if (data.code == 0) {
