...
export class PlanWeekComponent implements OnInit {
  ...
  // 点击某时间段获取相关信息
  getInTimeInfo(day){
    // {"oldmanId": "老人id","dayNum": "周几"}
    this.InTimeInfo = day;
    this.inTimeInfoLoading = true;
    this.httpReq.post('care_service/listWeek',null,{oldmanId:this.planid,dayNum:day},data=>{
  ...
  addServiceModelOk(){
    ...
    let sendData = {
      // {"oldmanId": "老人id","dayNum": "周几","projectIds":"服务项目ids","accountId": "账户id"}
    ...
    this.httpReq.post('care_service/saveWeek',null,sendData,data=>{
  ...
  getServiceList(renewPage:boolean = false){
    ...
    this.checkServiceArr = [];
    this.isPartSel = this.allChecked = false;
    this.httpReq.post("care_service/listForWeek", null, this.sendData, data => {
      this.addServiceModelLoading = false;
      if (data.code == 0) {
    ...
    if(id){
      this.modalService.confirm({
        ...
        nzOnOk: () => {
          ...
          this.httpReq.post("care_service/deleteWeek",null,sendData,data => {
            this.inTimeInfoLoading = false;
              if (data.code == 0) {
