...
@Component({
  ...
  styleUrls: ['./plan-week.component.css']
})
export class PlanWeekComponent implements OnInit {
  planName = ""; // 所属计划名称
  planid = ""; // 所属计划id
  loading = false; // loading
  timeLine = ["周一","周二","周三","周四","周五","周六","周日"]; // 时段列表
  ...
  // 点击某时间段获取相关信息
  getInTimeInfo(day){
    // {"planId":"服务计划Id","dayNum": "周几"}
    this.InTimeInfo = day;
    this.inTimeInfoLoading = true;
    this.httpReq.post('service_plan/listWeekPlan',null,{planId:this.planid,dayNum:day},data=>{
  ...
  addServiceModelOk(){
    ...
    let sendData = {
      // {"planId":"服务计划Id","dayNum": "周几","projectIds":"服务项目ids","accountId": "账户id"}
      planId: this.planid,
      dayNum: this.InTimeInfo,
      projectIds: projectIds.join(","),
      accountId: JSON.parse(localStorage.getItem("UserInfo")).data.id
    }
    this.addServiceModelLoading = true;
    this.httpReq.post('service_plan/saveWeekPlan',null,sendData,data=>{
  ...
  getServiceList(renewPage:boolean = false){
    ...
    this.checkServiceArr = [];
    this.isPartSel = this.allChecked = false;
    this.httpReq.post("service_plan/listForWeek", null, this.sendData, data => {
      this.addServiceModelLoading = false;
      if (data.code == 0) {
    ...
    if(id){
      this.modalService.confirm({
        ...
        nzOnOk: () => {
          ...
          this.httpReq.post("service_plan/deleteWeekPlan",null,sendData,data => {
            this.inTimeInfoLoading = false;
              if (data.code == 0) {
