...
export class PlanDayComponent implements OnInit {
  ...
  getDailyInfo(id){
    ...
    return new Promise((resolv)=>{
      this.httpReq.post("service_plan/startDayPlan",null,{id:id,accountId: JSON.parse(localStorage.getItem("UserInfo")).data.id},data=>{
        this.loading = false;
        if(data.code === 0){
  ...
  addTimeLineModelOk(){
    ...
    this.addTimeLineSendData.planId = this.planid;
    this.addTimeLineModelIsVisible = false;
    this.loading = true;
    this.httpReq.post('service_plan/saveDayInterval',null,this.addTimeLineSendData,data=>{
      this.loading = false;
      if(data.code === 0){
  ...
  getInTimeInfo(timeLineInfo){
    ...
    if(Object.hasOwnProperty.call(this.InTimeInfo,'id')){
      ...
      this.httpReq.post('service_plan/listDayPlan',null,{id:this.InTimeInfo['id']},data=>{
  ...
  addServiceModelOk(){
    ...
    this.httpReq.post('service_plan/saveDayPlan',null,sendData,data=>{
  ...
  getServiceList(renewPage:boolean = false){
    ...
    this.checkServiceArr = [];
    this.isPartSel = this.allChecked = false;
    this.httpReq.post("service_plan/listForDay", null, this.sendData, data => {
      this.addServiceModelLoading = false;
      if (data.code == 0) {
...
// ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑弹框添加服务相关↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑//
    ...
    if(id){
      this.modalService.confirm({
        ...
        nzOnOk: () => {
          this.inTimeInfoLoading = true;
          // {"intervalId":"时段id","projectIds":"服务项目ids","accountId": "账户id"}
          this.httpReq.post("service_plan/deleteDayPlan",null,{intervalId:this.InTimeInfo["id"],accountId: JSON.parse(localStorage.getItem("UserInfo")).data.id,projectIds: id},data => {
            this.inTimeInfoLoading = false;
              if (data.code == 0) {
                this.message.success("操作成功！");
                this.getInTimeInfo(this.InTimeInfo);
  ...
  deleltTime(data){
    ...
    this.loading = true;
    this.httpReq.post("service_plan/deleteDayInterval",null,{intervalId:data.id,accountId: JSON.parse(localStorage.getItem("UserInfo")).data.id},data=>{
      this.loading = false;
      if(data.code === 0){
        this.message.success("操作成功！");
        this.getDailyInfo(this.planid).then(data=>this.getInTimeInfo(data));
